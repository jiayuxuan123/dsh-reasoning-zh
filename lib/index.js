...
//   2. 注入：官方 DSH 不会为未声明 reasoning 的第三方模型显示「推理等级」控件，
//      也不会把所选档位写进请求体。本插件内置这一能力：未声明 reasoning 的
//      第三方模型自动获得推理等级控件（注入的档位直接给中文名），所选档位写入
//      出站请求体（字段名可配）。开箱即用，无需任何其它插件。
//
// 不修改任何官方 @deepseek-ai 包，包装方式与 DSH 自身的适配器机制同构，
// dsh 包升级后不会丢失。
...
// wire 注入会破坏其原生路径，跳过（原生适配器自行处理 reasoning 请求）。
const NATIVE_REASONING_CLASSES = new Set(["DeepSeekAdapter", "PiAiAdapter"]);
...
function normalizeConfig(raw) {
	...
	const levels = Array.isArray(src.levels) && src.levels.length > 0
		...
		: DEFAULT_LEVELS;
	// 未配置时默认 high；若用户自定义了 levels 且不含 high，则回退到第一个档位。
